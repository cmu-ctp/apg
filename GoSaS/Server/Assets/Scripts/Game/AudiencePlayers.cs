...
class AudienceActions{
	public static void DoHarvest( int curBuilding, int[] resources, ItemId[] items, ent pl ) {
		...
		if (rat < .45f) {
			...
			new ent() {
				sprite = TreatSys.resources[(int)harvestResources[curBuilding % 6, 0]], pos = new v3( pl.pos.x, -3f+pl.pos.y, rd.f(.5f)), scale = .4f, name="harvestResource", health=60, ignorePause=true, layer=Layers.UI,
				update = e => {
					e.health--;
					e.scale *= 1.01f;
			...
			new ent() {
				sprite = TreatSys.resources[(int)harvestResources[curBuilding % 6, 1]], pos = new v3( pl.pos.x, -3f+pl.pos.y, rd.f(.5f)), scale = .4f, name= "harvestResource", health=60, ignorePause=true, layer=Layers.UI,
				update = e => {
					e.health--;
					e.scale *= 1.01f;
					e.pos = e.pos + new v3(0, .002f, 0);
			...
			if( show) {
				new ent() {
					sprite = TreatSys.items[(int)harvestItems[curBuilding % 6]], pos = new v3( pl.pos.x, -3f+pl.pos.y, rd.f(.5f)), scale = .4f, name= "harvestItem", health=60, ignorePause=true, layer=Layers.UI,
					update = e => {
						e.health--;
						e.scale *= 1.01f;
						e.pos = e.pos + new v3(0, .002f, 0);
...
class AudienceExtra{
	public static void Ghost( v3 pos, ent leader, int team ) {
		float stopDist = rd.f(-4, -2), fadeOffset = rd.Ang(), tick = 0;
		var goalOffset = rd.Vec(-2, 2); 
		new ent() {
			sprite = ( team == 2 ? Art.Players.angelpurple.spr : Art.Players.angelyellow.spr ), pos = pos, scale = .35f, leader=leader,
			update = e => {
				tick++;
...
public class AudiencePlayerSys {
    ...
    public static Sprite[] heads = new Sprite[] { Art.Players.heads.head1.spr, Art.Players.heads.head2.spr, Art.Players.heads.head3.spr, Art.Players.heads.head4.spr, Art.Players.heads.head5.spr, Art.Players.heads.head6.spr, Art.Players.heads.head7.spr,
        Art.Players.heads.head8.spr, Art.Players.heads.head9.spr, Art.Players.heads.head10.spr, Art.Players.heads.head11.spr, Art.Players.heads.head12.spr, Art.Players.heads.head13.spr, Art.Players.heads.head14.spr, Art.Players.heads.head15.spr,
        Art.Players.heads.head16.spr, Art.Players.heads.head17.spr, Art.Players.heads.head18.spr, Art.Players.heads.head19.spr, Art.Players.heads.head20.spr};
	...
	void React(v3 pos, ImageEntry msg) {var delay = 30;new ent() { sprite = msg.spr, name="react", pos = pos, scale = 1, update = e => { delay--; if(delay <= 0) e.remove(); } };}
	...
	void Buddies(PlayerSys playerSys, TreatSys treats) {
        ...
        var anims = new Sprite[] { Art.Players.anims.build1.spr, Art.Players.anims.build2.spr, Art.Players.anims.defend1.spr, Art.Players.anims.defend2.spr, Art.Players.anims.dig1.spr, Art.Players.anims.dig2.spr,
            Art.Players.anims.help1.spr, Art.Players.anims.help2.spr, Art.Players.anims.jump1.spr, Art.Players.anims.jump2.spr, Art.Players.anims.shock1.spr, Art.Players.anims.shock2.spr,
            Art.Players.anims.sit1.spr, Art.Players.anims.sit2.spr, Art.Players.anims.walk1.spr, Art.Players.anims.walk2.spr, Art.Players.anims.bigslash1.spr, Art.Players.anims.bigslash2.spr,
            Art.Players.anims.smallslash1.spr, Art.Players.anims.smallslash2.spr, Art.Players.anims.item1.spr, Art.Players.anims.item2.spr, Art.Players.anims.bigslash2.spr};
        ...
        for ( var k2 = 0; k2 < 10; k2++ ) {
			...
			info.label = new ent(Art.Text.TextName.obj) { pos = new v3(0, 0, 0), text="", textColor=nameColor };
			var healthBar = new ent() { sprite = Art.UI.minihealthbarCenter.spr, pos = new v3(0, 0, 0), scale=.1f, color=new Color(.33f,0,0, .5f) };
			var labelBack = new ent(Art.Text.TextName.obj) { pos = new v3(0, 0, 0), text="", textColor=new Color(0,0,0,1), children = new List<ent> { info.label, healthBar }};
			info.headEnt = new ent() { pos = new v3(0, 0, 0), sprite = heads[(int)k], color = new Color(1, 1, 1, 1) };
            info.bodyEnt = new ent() { pos = new v3(0, 0, 0), scale = .505f, sprite = anims[14], color = nameColors[(int)k % 10] };
			...
			info.funcs = new BuddyFuncs {
				...
				doHarvest = () => AudienceActions.DoHarvest(info.curBuilding, resources, items, info.pl),
			...
			info.pl = new ent() {
				pos = new v3(goalx, -5, goalz), scale = .505f, health = maxHealth, children = new List<ent> { labelBack, info.bodyEnt, info.headEnt }, flipped=(k<10) ? false : true, leader= (k < 10) ? playerSys.playerEnt : ( playerSys.player2Ent != null ) ? playerSys.player2Ent:playerSys.playerEnt, team=(k<10)?Team.Player1:Team.Player2,
					name = "buddy"+k, inGrid=true, parent = bsrc, shadow=gameSys.Shadow(null, 1, .4f, 0 ), color= nameColors[(int)k % 10], ignorePause=true,
				onHurt = (e, src, dmg) => {
					...
					if (e.health > 0) {
						if (dmg.showDamage) {
							...
							if (soundTick - lastSoundTime > 10) { gameSys.Sound( Art.Sounds.complain1.snd, 1); lastSoundTime = soundTick; }
							React(e.pos + new v3(0, 0, -.2f),  Art.Reacts.ow );
							nameFlash = 1f;
							nameFlashColor = new Color(1, 0, 0, 1);}}
					else {
                        info.funcs.startAnim(AnimStyle.Die, true, false);
                        gameSys.Sound( Art.Sounds.dying1.snd, 1);
						info.label.text = "";
						labelBack.text = "";
						healthBar.active = false;
                        AudienceExtra.Ghost(e.pos, e.leader, info.team);
						React(e.pos + new v3(0, 0, -.2f), Art.Reacts.ugh);
				...
				update = e => {
                    ...
                    if (animFr < 0) animFr = 2;
                    if (animFr >= anims.Length) animFr = 2;
                    info.bodyEnt.sprite = anims[animFr];
                    var t = interp[animFr];
