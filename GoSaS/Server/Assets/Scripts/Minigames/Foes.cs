...
public class Foes:MonoBehaviour {
    ...
    public FriendlyDogGame dogGame;
    public RainGame rainGame;
    public KeyDoorGame keyDoorGame;}
...
public class RainGameLogic : Subgame{
    ...
    */
    ...
    public RainGameLogic( Foes theSrc, FoeSys theFoeSys ){
        f = theFoeSys;
        rainGuy = new SpawnEntry { icon = f.foes.beardguy, spawn = () => Make(), scale = .7f, message = "Deus Peduleus and Calceus Prime Approach!" };}
    public void Make() { }
}
...
public class DoorGameLogic : Subgame{
    ...
    public DoorGameLogic( Foes theSrc, FoeSys theFoeSys ){
        f = theFoeSys;
        chefGuy = new SpawnEntry { icon = f.foes.beardguy, spawn = () => Make(), scale = .7f, message = "Deus Peduleus and Calceus Prime Approach!" };}
    public void Make() { }
}
...
public class TrashGameLogic : Subgame{
    ...
    public TrashGameLogic( Foes theSrc, FoeSys theFoeSys ){
        f = theFoeSys;
        trashGuy = new SpawnEntry { icon = f.foes.beardguy, spawn = () => Make(), scale = .7f, message = "Deus Peduleus and Calceus Prime Approach!" };}
    public void Make() { }
}
...
public class BeardyGameLogic : Subgame{
    ...
    public BeardyGameLogic( Foes theSrc, FoeSys theFoeSys ){
        f = theFoeSys;
        beardGuy = new SpawnEntry { icon = f.foes.beardguy, spawn = () => BeardyGuy(), scale = .7f, message = "Deus Peduleus and Calceus Prime Approach!" };}
	void BeardyGuy() {
		for( var k = 0; k < 2; k++ ) {
			...
			i.lastNoiseTime = 0f; i.doShoot = true; i.angAnim = new DualWave(4, .025f); i.shakeAmount = 0f; i.shootDelay = 0; i.sprites = (k == 0) ? f.foes.shoes : f.foes.socks;
			new PoolEnt(f.foeEntPool) {
				sprite = (k == 0 ? f.foes.beardguy : f.foes.beardguy2), pos = new v3(rd.f(40), rd.f(-4, -7), rd.f(80, 100)), scale = 1f, name = "beardguy", inGrid=true, shadow= f.gameSys.Shadow(f.foes.shadow, f.foeEntPool, 3, 1, 0 ), team=Team.None,
				update = e => {
				...
				breathTouch = (e, user, info) => {
					...
					if(i.lastNoiseTime < f.tick - 10) { f.gameSys.Sound(f.foes.guySurpriseSound, 1); i.lastNoiseTime = f.tick;}
					i.shootDelay = 90;
...
public class PlantGameLogic : Subgame{
     ...
     * Rain
     */
    public PlantGameLogic( Foes theSrc, FoeSys theFoeSys ){
        f = theFoeSys;
        plantGuy = new SpawnEntry { icon = f.foes.plantguy, spawn = () => PlantGuy(), scale=2f, message="A Squadron of Utensil Vines Looms!" };}
    public void Run(int baseTime, SpawnSys spawnSys){ spawnSys.Add(f.foeOffset + baseTime, plantGuy); }
    void PlantGuy() {
		for( var k = 0; k < 4; k++ ) {
			var i = new FoeSys.foeInfo();
			i.startTime = f.tick; i.goal = new v3(0, 4, 30); i.horizontal = rd.f(15); i.vertOffset = rd.f(2, 6); i.tickOffset = rd.f(0, 300); i.horizontalDir = rd.Test(.5f) ? 1:-1; i.sprites = rd.Test(.5f) ? f.foes.spoons: f.foes.knives; i.inBack = false; i.delay = rd.i(0,1200);
				var backDist = rd.f(10,30);var shotTimer = rd.i(0,300);
			new PoolEnt(f.foeEntPool) {
				sprite = f.foes.plantguy, pos = new v3((rd.Test(.5f) ? -1:1) * 12 * i.horizontalDir, rd.f(-2, 5), 0), scale = .8f, flipped= i.horizontalDir > 0 ? false:true, name = "plantguy",shadow= f.gameSys.Shadow(f.foes.shadow, f.foeEntPool, 2, 1, 0 ), team = Team.None,
                update = e => {
...
public class FoeSys {
	...
	public FoeSys(Foes theFoes, GameSys theGameSys, PlayerSys thePlayerSys, AudiencePlayerSys theAudiencePlayerSys, TreatSys theTreatSys, ArtSet art ) {
        ...
        beardGame = new BeardyGameLogic(foes, this);
        dogGame = new DogGameLogic( foes,this, art );
        plantGame = new PlantGameLogic(foes, this);
    ...
    public void React(v3 pos, Sprite msg) {
		var delay = 30;
		new ent() { sprite = msg, pos = pos, scale = 1, update = e => { delay--; if(delay <= 0) e.remove(); } };}

	public void MakeShot(v3 pos, int j, ent src, Sprite[] sprites, float baseYVel = 0, bool ignorePlayer = false, float sc = 1 ) {
		...
		new PoolEnt(shotEntPool) {
			sprite = rd.Sprite(sprites), pos = pos, scale = rd.f(.3f, .4f)*1.5f * sc, name = "shot", inGrid=true, vel=new v3(j * .05f, -.04f + baseYVel, 0), shadow=gameSys.Shadow(foes.shadow, shotEntPool, 1, 1, 0 ), team=Team.None,
			update = e => {
	...
	void KeyDoor() {
		...
		new PoolEnt(foeEntPool) {
			sprite = foes.foeTrash, pos = new v3(rd.f(40), rd.f(-4, -7), rd.f(80, 100)), scale = 1f, name = "chaser1", inGrid = true, shadow = gameSys.Shadow(foes.shadow, foeEntPool, 3, 1, 0), team = Team.None,
			update = e => {
				i.shootDelay--;
	...
	void NervousTouch() {
		for( var k = 0; k< 5; k++) { 
			...
			new PoolEnt(foeEntPool) {
				sprite = foes.foeMustache, pos = new v3(i.goal.x, i.goal.y, rd.f(80, 100)), scale = .3f, name = "nervous", inGrid = true, shadow = gameSys.Shadow(foes.shadow, foeEntPool, 3, 1, 0), team = Team.None,
				update = e => {
					i.shootDelay--;
					if (TryLeave(e, i.startTime, ref i.goal, playerSys.playerEnt.pos + nm.v3z(.3f))) return;
					i.delay--;
		...
		for( var k = 0; k< 12; k++) { 
			...
			new PoolEnt(foeEntPool) {
				sprite = foes.foeMustache, pos = new v3(i.goal.x, i.goal.y, rd.f(80, 100)), scale = .1f, name = "nervousExploder", inGrid = true, color= new Color(0,0,0,1), shadow = gameSys.Shadow(foes.shadow, foeEntPool, 3, 1, 0), team = Team.None,
				update = e => {
					if (TryLeave(e, i.startTime, ref i.goal, playerSys.playerEnt.pos + nm.v3z(.3f))) return;
					i.delay--;
					if (i.delay > 0) return;
	...
	void AbsentMinded() {
		...
		for( var k = 0; k< 12; k++) { 
			...
			i.startTime = tick; i.goal = new v3(rd.f(-12,12), -1, 30); i.angAnim = new DualWave(4, .025f); i.shakeAmount = 0f; i.shootDelay = 0; i.sprites = foes.shoes; i.slide = new v3(0, 0, 0); i.delay = k * .5f*60;
			new PoolEnt(foeEntPool) {
				sprite = foes.foeMustache, pos = new v3(i.goal.x, i.goal.y, rd.f(80, 100)), scale = .1f, name = "absentExploder", inGrid = true, color= new Color(0,0,0,1), shadow = gameSys.Shadow(foes.shadow, foeEntPool, 3, 1, 0), team = Team.None,
				update = e => {
					if (TryLeave(e, i.startTime, ref i.goal, playerSys.playerEnt.pos + nm.v3z(.3f))) return;
					i.delay--;
					if (i.delay > 0) return;
	...
	void TrashGuy() {
		for( var k = 0; k < 5; k++ ) {
			...
			i.baseInit( tick  );
			new PoolEnt(foeEntPool) {
				sprite = foes.foeTrash, pos = new v3(rd.f(40), rd.f(-4, -7), rd.f(80, 100)), scale = .5f, flipped=rd.Test(.5f), name = "trashguy",shadow=gameSys.Shadow(foes.shadow, foeEntPool, 3, 1, 0 ), team = Team.None,
				update = e => {
	...
	void MicrowaveGuy() {
		for( var k = 0; k < 5; k++ ) {
			...
			i.baseInit(tick);
			new PoolEnt(foeEntPool) {
				sprite = foes.foeMicrowave, pos = new v3(rd.f(40), rd.f(-4, -7), rd.f(80, 100)), scale = 1f, flipped=rd.Test(.5f), name = "microwaveguy",shadow=gameSys.Shadow(foes.shadow, foeEntPool, 3, 1, 0 ), team = Team.None,
				update = e => {
					if (TryLeave(e, i.startTime, ref i.goal, new v3(i.horizontal, i.vertOffset, playerSys.playerEnt.pos.z + .3f))) return;
					if((tick + i.tickOffset) % 600 < 200) {if((tick + i.tickOffset) % 600 > 100 && i.doShoot) { i.doShoot = false; } i.goal = new v3(i.horizontal, i.vertOffset, 3);}
					else { i.doShoot = true; i.goal = new v3(i.horizontal, i.vertOffset, i.zDepth);}
	...
	void MustacheGuy() {
		for( var k = 0; k < 5; k++ ) {
			...
			i.baseInit(tick);
			new PoolEnt(foeEntPool) {
				sprite = foes.foeMustache, pos = new v3(rd.f(40), rd.f(-4, -7), rd.f(80, 100)), scale = 1f, flipped=rd.Test(.5f),name = "mustacheguy",shadow=gameSys.Shadow(foes.shadow, foeEntPool, 3, 1, 0 ), team = Team.None,
				update = e => {
					if (TryLeave(e, i.startTime, ref i.goal, new v3(i.horizontal, i.vertOffset, playerSys.playerEnt.pos.z + .3f))) return;
