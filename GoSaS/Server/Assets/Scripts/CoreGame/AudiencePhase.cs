...
public class AudiencePhase {
	...
	class HudShared {
        ...
        public HudShared( AudiencePhaseInfo infoSrc, Transform theTransform, GameObject theTextName, Sprite thePlayerHighlight, Material theGlowMaterial) {
	...
	class PlayerHUD {
		...
		public PlayerHUD( HudShared theSrc, float scs, int sideSet, Players players) {
			src = theSrc;
			...
			face = make(scs * 5, yadd + 3, players.heads[0], 1.5f, sideSet);
			action = makeAction(scs * 3f, yadd + 3.5f, players.actions[0], 2f, sideSet);
			health = makeHealth(scs * 5.5f, yadd + 2.5f, players.healthBar, .3f, sideSet); health.color = new Color(1, 0, 0, 1);
			buildingpic = makeBuilding(scs * 6.5f, yadd + 2, players.buildings[0], .65f, sideSet);}
		...
		public ent makeHealth(float x, float y, Sprite spr, float sc, int set) {
			return new ent() {
				ignorePause = true, parentTrans = src.transform, scale = sc, pos = new v3(x, y, 10), sprite = spr, health = 0, layer = Layers.UI, color = new Color(1, 0, 0, 1),
				update = e2 => {
					var mul = (set == 1) ? 1 : -1;
					if (set == 1 && !src.info.showSet1) e2.scale = 0;
					else if (set == 2 && !src.info.showSet2) e2.scale = 0;
		...
		public void ChangeFocus( APlayerInfo g2, Players players ) {
			actionFail = 0;
			if (g2 != null && g2.pl.health > 0) {
				...
				buildingpic.sprite = players.buildings[g2.curBuilding%6];
				...
				face.sprite = players.heads[g2.headID];
				if (g2.itemId != ItemId.Unset) action.sprite = players.items[(int)g2.itemId];
				else if (g2.buildingActionId != BuildingActionID.Unset) action.sprite = players.buildingActions[(int)g2.buildingActionId];
				else action.sprite = players.actions[(int)g2.actionId]; }
			action.health = 240;}}
    ...
    static void MakeMessage( GameObject textName, Transform transform, int roundNumber, string message, Action onUpdate, Action onEnd ) {
		new ent(textName) {
	...
	static void ChangeAudienceFocus(HudShared src, PlayerHUD hud1, PlayerHUD hud2, APlayerInfo[,] grid, Players players) {
	...
	static void RunAudiencePhase( AudiencePhaseInfo infoSrc, AudiencePhaseStatus aStatus, APlayerInfo[,] grid, Players players, Backgrounds backgrounds, Transform theTransform, GameObject theTextName, Sprite thePlayerHighlight, Material theGlowMaterial) {
		var src = new HudShared( infoSrc, theTransform, theTextName, thePlayerHighlight, theGlowMaterial);

		var yadd = -7;

		new ent() { ignorePause = true, parentTrans = src.transform, scale = 10, pos = new v3(0, yadd + 2 + 1, 9), sprite = players.actionBkg, health=0, layer = Layers.UIBkg, color = new Color(1, 0, 0, .8f), ang=177+180,
			update = e2 => {
				if (!src.info.atRemove) { var v = e2.pos; nm.ease(ref v, new v3(0, yadd + 2 + 1, 10), .1f); e2.pos = v; }
				else { var v = e2.pos; nm.ease(ref v, new v3(0, -10, 10), .1f); e2.pos = v; }
				if (e2.pos.y < -9f) e2.remove(); }};
		new ent() { ignorePause = true, parentTrans = src.transform, scale = 10, pos = new v3(0, 6.5f, 9), sprite = players.actionBkg, health=0, layer = Layers.UIBkg, color = new Color(1, 0, 0, .8f), ang=177,
			update = e2 => {
				if (!src.info.atRemove) { var v = e2.pos; nm.ease(ref v, new v3(0, 6.5f, 10), .1f); e2.pos = v; }
				else { var v = e2.pos; nm.ease(ref v, new v3(0, 10, 10), .1f); e2.pos = v; }
				if (e2.pos.y > 9f) e2.remove(); }};

		var hud1 = new PlayerHUD( src, -1, 1, players);
		var hud2 = new PlayerHUD( src, 1, 2, players);
	
		new ent() {
			ignorePause = true, scale = 2f, pos = new v3(0, 3, 10), sprite = src.playerHighlight, material= src.glowMaterial, color = new Color(1, 1, 1, .5f), 
			update = e2 => {
		...
		new ent() {
			...
			update = e2 => {
				...
				if (src.curDelay <= 0) ChangeAudienceFocus(src, hud1, hud2, grid, players);
