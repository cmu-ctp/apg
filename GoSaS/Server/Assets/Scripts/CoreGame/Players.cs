...
class PlayerAttacks
{
	public static void MakeBreath( GameSys gameSys, Sprite[] clouds, ent owner, List<Action<v3, float, float, float>> blow, List<Action<v3, float, float, float>> inhale, List<Action<v3, float, float, float>> inhaleBig ) {
		...
		for( var k = 0; k < 20; k++ ) {
			...
			var blowing = false;
			var f = new ent() {
				sprite = rd.Sprite(clouds), scale=0, name="playerbreath", parent = src,
				update = e => {
...
class EndPlaque{
    public static void MakeEndPlaque( Sprite introPlaque, Transform transform ){
        new ent() { ignorePause = true, sprite = introPlaque, parentTrans = transform, scale = 1.3f * 0f, pos = new v3(0, 0, 10), health = 1,
            update = e2 => {
...
public class PlayerSys {
	...
	public void Setup(GameSys theGameSys, Players thePlayers, FoeSys foeSys, ReactSys theReactSys, Sprite introPlaque, Transform transform) {
		gameSys = theGameSys;
		players = thePlayers;
		reactSys = theReactSys;

		Player(1, foeSys, introPlaque, transform);
		Player(2, foeSys, introPlaque, transform);}

	void Player(int id, FoeSys foeSys, Sprite introPlaque, Transform transform) {
		...
		var pl = new ent() {
			sprite = pic, pos = nm.v3x( startingX ), scale = 1.5f, flipped=(id == 2) ? true : false, vel = new v3(0, 0, 0), knockback = new v3(0, 0, 0), name="player"+id, inGrid=true, shadow=gameSys.Shadow(players.shadow, null, 1, 1, 0 ), team= (id==1)?Team.Player1:Team.Player2,
			update = e => {
				tick++;
		...
		PlayerAttacks.MakeBreath( gameSys, players.clouds, pl, blow, inhale, inhaleBig );
		var halotick = 0;
		new ent() {
