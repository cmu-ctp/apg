...
class AudienceSegment{
	...
	class PlayerHUD {
		...
		ent make(float x, float y, Sprite spr, float sc, int set) {
			return new ent() {
		...
		ent makeTx(float x, float y, string tx, int set, bool isBack) {
			return new ent(Art.Text.TextName.obj) {
				...
				update = e2 => {
					...
					if (e2.pos.y < -9f) e2.remove();}};}
		ent makeAction(float x, float y, Sprite spr, float sc, int set) {
			return new ent() {
				ignorePause = true, parentTrans = src.transform, scale = sc, pos = new v3(x, y, 10), sprite = spr, health = 0, layer = Layers.UI,
				update = e2 => {
		...
		ent makeHealth(float x, float y, float sc, int set) {
			return new ent() {
				...
				update = e2 => {
					...
					if (e2.pos.y < -9f) e2.remove();}};}
		ent makeBuilding(float x, float y, Sprite spr, float sc, int set) {
			return new ent() {
	...
	class ActionInfo {
		...
		public void DoAction( PlayerHUD hud, BuildingActions buildingActions, Items items ) {
			...
			if (item != ItemId.Unset) { g.funcs.useUpSelectedItem(); items.DoItem(item, 1, g.pl.pos, g);}
			if (building != BuildingActionID.Unset) {
				if (hud.curBuildingUsed) { hud.actionFail = 1; }
				else { g.funcs.useSelectedBuildingAction(); buildingActions.DoBuilding(building, 1, g.pl.pos, g); }
				hud.curBuildingUsed = true;
				hud.buildingpic.color = new Color(.3f, .3f, .3f, 1);}}}

	public static void RunAudiencePhase( AudiencePhaseInfo infoSrc, AudiencePhaseStatus aStatus, APlayerInfo[,] grid, BuildingActions buildingActions, Items items, Transform theTransform) {
        var src = new HudShared(infoSrc, theTransform);
        var hud1 = new PlayerHUD(src, -1, 1);
        var hud2 = new PlayerHUD(src, 1, 2);

	    Action<int> RunActions = curGrid => { 
            Action<ActionInfo, bool, bool, bool> RunAnim = ( v, successfulAttack, successfulRetaliate, hurt ) => {
                switch( v.g.actionId){
        ...
        Action run = () => { 
		    ...
		    new ent() {
			    ...
			    update = e2 => {
				    ...
				    src.actionDelay--;
				    if (src.actionDelay == 0) RunActions( src.curGrid );
				    if (src.curDelay <= 0) ChangeAudienceFocus();
				    else RunNormalFrame();
...
public class AudiencePhase {
	public static void EndOfRoundCamera( v3 cameraPos, ref v3 lastLookAtPos, ref v3 lastLookFromPos, ref v3 lookPos, Transform transform) {
    ...
    static float MakeBeforeAudience( Transform transform, int roundNumber ){
        var roundEndMessageTime = 1;
        var roundStartMessageTime = roundEndMessageTime+2.5f;
        var roundStatsTime = roundStartMessageTime + .5f;
        var oldRoundFadeOutTime = roundStatsTime + 2.5f;
        var audienceTurnMessageTime = oldRoundFadeOutTime+1.5f;

        Action RoundEndMessage = () => {
            var tick = 0;
            new ent(Art.Text.TextName.obj) {
        ...
        Action RoundStats = () => {
            var tick = 0;
            new ent() {
                ignorePause = true, parentTrans = transform, ang=rd.f(-.5f,.5f), scale = 1f, pos = new v3(0, -50, 20), health = FullGame.ticksPerSecond * 2, sprite = Art.UI.howtoplay.spr, color = new Color(1, 1, 1, 1), name = "statsbkg", 
                update = e2 => {
                    tick += 2;
                    if (tick < FullGame.ticksPerSecond * roundStatsTime) return;
                    var goal = e2.pos * .9f + .1f * new v3(0, 0,10);
                    e2.pos = goal;
                    e2.ang = e2.ang * .9f;
                    if (tick > FullGame.ticksPerSecond * roundStatsTime + 210) e2.remove(); } }; };
        Action RoundEndFadeOut = () => {
            var tick = 0;
            new ent() {
        ...
        RoundStartMessage();
        AudienceTurnMessage();
        return audienceTurnMessageTime + 2.5f;}

	static void MakeAfterAudience(AudiencePhaseStatus aStatus, Transform transform, int roundNumber, APlayerInfo[,] grid, Action audienceActionsEnded) {
        Action DoStreamerMessage = () => {
		...
		new ent(Art.Text.TextName.obj) {
			...
			update = e2 => {
				...
				if (textFade(e2, .1f)) { DoStreamerMessage(); e2.remove();}}};}
    ...
    public static void MakeRoundEnd(AudiencePhaseStatus aStatus, int roundNumber, APlayerInfo[,] grid, Action audienceActionsEnded, Transform theTransform, BuildingActions buildingActions, Items items ) {
        ...
        var audienceActionsTime = MakeBeforeAudience(theTransform, roundNumber);
		...
		new ent() { ignorePause = true,
			update = e => {
				info.tick++;
				if (info.tick == FullGame.ticksPerSecond * audienceActionsTime) AudienceSegment.RunAudiencePhase(info, aStatus, grid, buildingActions, items, theTransform);
				if (info.doingAction ==false) { aStatus.midpointTimer = 1; }
				else if (!info.atRemove) { aStatus.midpointTimer = .5f;}
				else {MakeAfterAudience( aStatus, theTransform, roundNumber, grid, audienceActionsEnded );e.remove();}}};}}