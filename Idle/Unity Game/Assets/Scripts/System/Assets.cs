...
public class AssetLister{
        ...
        foreach (string d in Directory.GetDirectories(sDir)){
            ...
            foreach (string f in Directory.GetFiles(d)){
                ...
                if (Path.GetExtension(f) == ".png" || Path.GetExtension(f) == ".jpg" || Path.GetExtension(f) == ".tga" || Path.GetExtension(f) == ".prefab"){
                    ...
                    files.Add(name);}
                if (Path.GetExtension(f) == ".wav" || Path.GetExtension(f) == ".mp3" || Path.GetExtension(f) == ".mod" || Path.GetExtension(f) == ".s3m" || Path.GetExtension(f) == ".xm"|| Path.GetExtension(f) == ".it" ){
    ...
    public static void BuildFileList(){
        ...
        dirs.Append("}");
        File.WriteAllText(Application.dataPath + Path.DirectorySeparatorChar + "Scripts\\Game\\AssetSet.cs", dirs.ToString());}}