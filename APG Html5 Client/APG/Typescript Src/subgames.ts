...
class WaitingForJoinAcknowledement{
	...
	static make(sys: APGSys): void {
		...
		sys.messages = new MessageHandler({
			onJoin: () => { endOfRoundSound.play(); MainPlayerInput.make(sys); return true; }
		});
...
class ShowSubmitted {
	...
	static make(sys: APGSys, getRoundNumber: () => number):void {
...
class MainPlayerInput{
	...
	static make(sys: APGSys): void {
		...
		sys.messages = new MessageHandler({
			...
			clientUpdate: () => { },
			startSubmitInput: () => {
				ShowSubmitted.make(sys, () => roundNumber);
