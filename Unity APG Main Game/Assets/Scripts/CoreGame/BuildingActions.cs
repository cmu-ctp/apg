...
public class BuildingActions {
    ...
    public BuildingActions( GameSys theGameSys, Buildings theBuildingSet, PlayerSys thePlayerSys, TreatSys theTreatSys){
        gameSys = theGameSys;
	...
	void makeBroccoli(v3 pos, int side) {
		new ent() { sprite = buildingSet.cannonClouds[rd.i(0,4)], pos = new v3( pos.x+rd.f(-.5f,.5f), pos.y + rd.f(-.5f, .5f), .8f + rd.f(-.05f, .05f)), flipped=rd.Test(.5f), name="cowstink", inGrid=true, scale=.7f,
	...
	void doSun( v3 pos ) {
		var vel = rd.f(.1f, .2f ); var spin = rd.f(-6,6); var firstBounce = true; var xvel = 0f; var numBounces = 0; var id = rd.i(0, 10);
		new ent() {
			sprite = buildingSet.flowers[rd.i(0, 2)], pos = new v3( pos.x+ rd.f(1), pos.y+rd.f(.5f), rd.f(.5f)), scale = .3f, flipped=rd.Test(.5f), name="basicTreat", inGrid=true, 
			update = e => {
				e.pos += new v3( xvel,vel, 0);
				vel -= .005f;
				e.ang += spin;
