	...
	void RunDebug(PlayerSet playerSet, APGSys apg) {
		...
		new ent( gameSys ) {
			update = e => {
				...
				if( tick > 60 * 7 ) {
					tick = 0;
					for( var k = 0; k < names.Length; k++ ) {apg.WriteLocal( names[k], "upd", new APGBasicGameLogic.SelectionParms{ choices= new int[] {rd.i(0,6),rd.i(0,3),rd.i(0,6), rd.i(0, 7), rd.i(0, 6), 1 } } );}}}};}
	...
	void Buddies(PlayerSet playerEvents, PlayerSys playerSys) {
		...
		for ( var k2 = 0; k2 < 20; k2++ ) {
			...
			playerEvents.RegisterHandler(new AudiencePlayerEventsHandler {
				...
				getPos = () =>pl.pos,
				doMove = () => { goalx = delayGoalX; goalz = delayGoalZ; },
				showActionStart = frames => {/*nameFlash = 1f; nameFlashColor = new Color(1, 1f, 1f, 1);*/ },
				onRoundEnd = () => {
					parms = bufferedParms;
					...
					goalLayer = parms[1];
					actionId = parms[2];},
				updateToClient = ( apg, userName ) => { apg.WriteToClients("pl", new PlayerUpdate { nm = userName, st=new int[] { pl.health, 10, buildingGoal, goalLayer }, rs= resources });}});
			
			pl = new ent(gameSys) {
				sprite = players.anims[14], pos = new v3(goalx, -5, goalz), scale = .505f, health = 5, children = new List<ent> { labelBack, head }, flipped=(k<10) ? false : true, leader= (k < 10) ? playerSys.playerEnt : ( playerSys.player2Ent != null ) ? playerSys.player2Ent:playerSys.playerEnt,
