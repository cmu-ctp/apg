...
namespace APG {
	public struct PlayerEndOfRoundInfo { public v3 pos; public string name; public Color nameColor; public Color stanceColor; public Color actionColor; public Sprite stanceIcon; public string stanceName; public string actionName; }
	public class PlayerSet {
		...
		int activePlayers = 0;
		public void RegisterHandler( AudiencePlayerEventsHandler events ) { playerEvents.Add( events ); }
		public int PlayerCount() { return activePlayers;}
		bool ValidPlayer( int playerNumber ) { if( playerNumber < 0 || playerNumber >= playerEvents.Count || playerEvents[playerNumber] == null )return false; return true;}
		public AudiencePlayerEventsHandler GetPlayerEvents( int playerNumber ) { if( !ValidPlayer( playerNumber ) )return nullEvents; return playerEvents[playerNumber];}
		public string PlayerName( int playerNumber ) { if( !ValidPlayer( playerNumber ) )return ""; return playerNames[playerNumber]; }
		public int[] PlayerInput( int playerNumber ) { if( !ValidPlayer( playerNumber ) )return null; return playerInput[playerNumber];}
		public bool AddPlayer( string playerName ) {
			...
			activePlayers++;
			return true;}
		public int GetPlayerID( string user) {return playerMap[user];}
		public bool SetPlayerInput( string user, int[] parms ) {
			...
			if( id < 0 || id >= playerInput.Count ) { Debug.Log( "SetPlayerInput: Player ID is out of range: " + id + " (should be between 0 and " + playerInput.Count + ")" );}
			playerInput[id] = parms;
			return true;}
		public void RoundUpdate() { for( var k = 0; k < playerNames.Count; k++ ) { playerEvents[k].onRoundEnd(); }}
		public void UpdatePlayersToClients( APGSys apg ) {for( var k = 0; k < playerNames.Count; k++ ) {playerEvents[k].updateToClient( apg, playerNames[k] );}}
		public List<PlayerEndOfRoundInfo> GetEndOfRoundInfo() {
			List<PlayerEndOfRoundInfo> infos = new List<PlayerEndOfRoundInfo>();
			for( var k = 0; k < playerNames.Count; k++ ) {infos.Add( playerEvents[k].getEndOfRoundInfo() );}
			infos.Sort((x, y) => x.pos.x.CompareTo(y.pos.x));
			return infos;}
		AudiencePlayerEventsHandler[,] playerGrid = null;
		AudiencePlayerEventsHandler[,] oldPlayerGrid = new AudiencePlayerEventsHandler[12, 3];
		public AudiencePlayerEventsHandler[,] GetPlayerGrid() { return oldPlayerGrid; }
		public void InitGrid() {
			playerGrid = new AudiencePlayerEventsHandler[12, 3];
			for (var k = 0; k < playerNames.Count; k++) {
				int building = playerEvents[k].getGoalBuilding();
				int layer = playerEvents[k].getLayer();
				playerGrid[building, layer] = playerEvents[k];
				Debug.Log("at " + building + " " + layer + " adding " + playerEvents[k].getName());}}
		public void SetGoalPositions() {
			...
			for (var k2 = 0; k2 < playerNames.Count; k2++) {
				...
				int building = playerEvents[k].getGoalBuilding();
				int layer = playerEvents[k].getLayer();
				if (playerGrid[building, layer] == null) playerGrid[building, layer] = playerEvents[k];
				else {
