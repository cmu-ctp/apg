...
public class SpawnSys {
	...
	public void Add(int spawnTime, SpawnEntry src) { spawnSet.Add(new SpawnEntry { icon=src.icon, time=spawnTime*60, spawn=src.spawn, iconYOffset=src.iconYOffset, message=src.message }); }
	public void Sort() { spawnSet.Sort((x, y) => x.time.CompareTo(y.time)); }
	public void Update(int time) {
		if(curEntry >= spawnSet.Count) return;
		if(time > spawnSet[curEntry].time - 60 * 2.5f) {
