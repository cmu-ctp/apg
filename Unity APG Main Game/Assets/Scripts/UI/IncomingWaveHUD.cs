	...
	/*	void MakeCutsceneBorders( GameSys gameSys, IncomingWaveHUD assets, ent uiBkg ) {
			...
			new ent(gameSys) { sprite=assets.cutsceneEdges, ang=180, active=true, name="cutsceneEdge", parent = uiBkg,  pos = new v3(0, 4.9f-4.3f, 0 ), scale=4.1f, ignorePause=true,
				update = e => {
					...
					if( timer == 0 ) { nm.ease( ref offset, 0, .1f ); }
					else { nm.ease( ref offset, 1, .1f ); }
					tc++;
					if( tc == 30 ) { tc = 0; Debug.Log( "" + offset  );}
					bottom.pos = new v3(0, -4.9f-13.3f - offset*2, 0 );
					e.pos = new v3(0, 4.9f-4.3f + offset*2, 0 );}};}*/
	void MakeQRCode(GameSys gameSys, IncomingWaveHUD assets, MonoBehaviour src, Texture2D qrCode ) {
		Sprite spr = Sprite.Create(qrCode, new Rect(0,0,256, 256), new Vector2(.5f, .5f));
		new ent(gameSys) { sprite = spr, pos = new v3(0, 0f, 0), scale = 1, active = true, name = "QRCode", parentMono = src };}
